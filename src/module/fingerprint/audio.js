...
export default {
    ...
    async getInfo(){
        return new Promise(function(resolve){
            try{
                let OfflineAudioContext = globalThis.OfflineAudioContext || globalThis.webkitOfflineAudioContext;
                if(!OfflineAudioContext){   // 非浏览器/不支持环境
                    resolve(getMD5(''));
                    return;
                }
